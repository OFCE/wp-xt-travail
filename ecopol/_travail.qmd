  ...
  ofce-html:
    code-links:
      ...
      - text: Salaires trimestriels
        icon: file-code
        href: salaires.r
      - text: Structure des rémunérations
        icon: file-code
        href: earn.r
      - text: Indicateurs du marché du travail
        icon: file-code
        href: precarite.r
      - text: Non salariés
        icon: file-code
...
salaires <- source_data('R/salaires.r') |> 
  ...
  "<b>{lbl(geo)}</b>
  {date_trim(time)}
  Salaire nominal : {round(w*pc,1)} k€ courants
  Salaire réel : {round(w,1)} k€ 2023"))
pp <- source_data("R/nace.r")$pays1
sal <- ggplot(salaires |> filter(time >= "1995-01-01", geo %in% pp  )) +
  aes(x=time, y=w, col = geo) +
  ...
  geom_ribbon(aes(ymin = wb, ymax = w, fill = geo, group = geo),
  ...
  ofce_caption(
    ...
    calcul de l'auteur.",
    code =  "github.com/xtimbeau/travail/R/salaires.r",
...
dependants <- source_data("R/dependants.r") |> 
  mutate(tooltip = glue(
    "<b>{lbl(geo)}</b>
...
psal <- ggplot(naa) +
 ...
 calcul de l'auteur.",
    code =  "github.com/xtimbeau/travail/R/va.r",
...
return <- ggplot(naaa) +
 ...
 comptes nationaux par secteurs institutionels (nasa_10_nf_tr) pour l'IS (D51), comptes d'actifs physique  (nama_10_nfa_st), téléchargés le 15 janvier 2025, calcul de l'auteur.",
    code = "github.com/xtimbeau/travail/R/var.r",
    champ = "Branches marchandes hors services immobiliers (L dans la NACE r2)",
